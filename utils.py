...
class Minter(Help):
    def __init__(self, privatekey, chain, count, delay, mode):
        ...
        self.drop_address = Web3.to_checksum_address('0xB6432d111bc2A022048b9aEA7C11b2d627184bdD')
        self.mode = mode
    ...
    def mint(self):
        ...
        while True:
            try:
                ...
                if status == 1:
                    logger.info(
                        f'{self.address}:{self.chain} - успешно заминтил {self.count} azure {scans[self.chain]}{self.w3.to_hex(hash_)}...')
...
class Bridger(Help):
    def __init__(self, privatekey, chain, to, delay, api, mode):
        ...
        self.nft_address = Web3.to_checksum_address('0xB6432d111bc2A022048b9aEA7C11b2d627184bdD')
    ...
    def check_nft(self):
        if self.mode == 0 and self.chain != 'opti':
            ...
            try:
                ...
                if id_:
                    logger.success(f'{self.address} - azure {id_} nft founded on {self.chain}...')
                    return id_
            except Exception as e:
                logger.error(f'{self.address} - azure nft not in wallet...')
                return False
            ...
            for chain in ['avalanche', 'polygon', 'bsc']:
                ...
                try:
                    ...
                    id_ = int(result['result'][0]['token_id'])
                    if id_:
                        logger.success(f'{self.address} - azure {id_} nft founded on {chain}...')
    ...
    def bridge(self):
        if self.mode == 0:
            ...
            if data:
                ...
                return self.privatekey, self.address, 'azure nft not in wallet'
        ...
        while True:
            ...
            try:
                ...
                if status == 1:
                    logger.success(
                        f'{self.address}:{self.chain} - successfully bridged azure {nft_id} to {self.to} : {self.scan}{self.w3.to_hex(hash_)}...')
                    self.sleep_indicator(self.delay)
                    return self.privatekey, self.address, 'success'
            except Exception as e:
                error = str(e)
